...
python utils/make_configs.py --batch_size 128 --learning_rate 0.001 --max_epoch 30 --sound_length 2400 --input_feature logmel logmel --dataset_name pose_regression_kfold_subject_1 pose_regression_kfold_subject_2 pose_regression_kfold_subject_3 pose_regression_kfold_subject_4 pose_regression_kfold_subject_5 pose_regression_kfold_subject_6 pose_regression_kfold_subject_7 pose_regression_kfold_subject_8 --model speech2pose_ad speech2pose_ad --smooth_loss True --gan std std --ratio 1.0
files="./result/*std*"
for filepath in $files; do
    if [ -d $filepath ] ; then
        ...
        python train_adversarial.py "${filepath}/config.yaml" --use_wandb
        python evaluate_adversarial.py "${filepath}/config.yaml" validation
